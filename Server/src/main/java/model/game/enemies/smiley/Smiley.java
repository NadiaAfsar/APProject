...
public class Smiley extends Enemy implements Movable {
    ...
    private EpsilonModel projectileEpsilon;
    private EpsilonModel squeezingEpsilon;
    public Smiley(Point center, double velocity, GameManager gameManager, ArrayList<EpsilonModel> epsilons) {
        super(center, velocity, gameManager, epsilons.get(0));
    ...
    public Direction getDirection() {
        ...
        if (projectile) {
            Direction direction = new Direction(center, projectileEpsilon.getCenter());
            Direction direction1 = new Direction();
            direction1.setDx(direction.getDy()*10);
            direction1.setDy(-direction.getDx()*10);
            return direction1;
        }
        int random = (int)Math.round(Math.random()*(epsilons.size()-1));
        Point epsilonCenter = epsilons.get(random).getCenter();
    ...
    private void checkProjectile(){
        Point epsilonCenter = projectileEpsilon.getCenter();
        ...
        if (Math.abs(distance-300) > 50 || hands.size()==0) {
            ...
            for (int i = 0; i < hands.size(); i++){
                hands.get(i).shoot(projectileEpsilon);
            }
        }
    }
    private void squeezeSetPosition(){
        for (int i = 0; i < hands.size(); i++) {
            ...
            double x = squeezingEpsilon.getFrame().getX()+j*(hand.getFrame().getWidth()/2)+squeezingEpsilon.getFrame().getWidth()*((j+1)/2);
            double y = squeezingEpsilon.getFrame().getY()+20;
            hand.setCenter(new Point(x, y));
            hand.getFrame().setRigid(true);
            hand.setSusceptible(false);
        }
        setCenter((new Point(squeezingEpsilon.getFrame().getX()+squeezingEpsilon.getFrame().getWidth()/2, center.getY())));
        if (Math.abs(myFrame.getY()+ myFrame.getHeight()-squeezingEpsilon.getFrame().getY()) > 50){
            squeezing = false;
        }
    }
    ...
    private void firstPhaseAttack() {
        if (hands.size() != 0) {
            for (int i = 0; i < epsilons.size(); i++) {
                MyFrame epsilonMyFrame = epsilons.get(i).getInitialMyFrame();
                if (Math.abs(myFrame.getY() + myFrame.getHeight() - epsilonMyFrame.getY()) < 30 && (Calculations.isInDomain(myFrame.getX(),
                    ...
                    squeezingEpsilon = epsilons.get(i);
                    squeeze();
                    return;
                }
                else {
                    Point epsilonCenter = epsilons.get(i).getCenter();
                    double distance = Calculations.getDistance(epsilonCenter.getX(), epsilonCenter.getY(), center.getX(), center.getY());
                    if (Math.abs(distance - 300) < 10) {
                        logger.debug("projectile");
                        projectileEpsilon = epsilons.get(i);
                        projectile();
                        return;
                    }
                }
    ...
    private void squeeze() {
        ...
        setCenter((new Point(squeezingEpsilon.getFrame().getX()+squeezingEpsilon.getFrame().getWidth()/2,
                squeezingEpsilon.getFrame().getY()- myFrame.getHeight()/2)));
        susceptible = true;
    ...
    public void bulletShot(BulletModel bulletModel, EpsilonModel epsilon){
        if (!squeezing) {
    ...
    private void vomit() {
        ...
        for (int i = 0; i < 5; i++){
            ...
            MyFrame myFrame = epsilons.get(random).getInitialMyFrame();
    ...
    public void die() {
        ...
        for (int i = 0; i < epsilons.size(); i++) {
            epsilons.get(i).setXP(epsilons.get(i).getXP() + 250);
        }
